...
class ControlFertilizantes(Producto):
    ...
    def ultima_aplicacion(self, value):
        try:
            self._ultima_aplicacion = datetime.strptime(value, "%Y-%m-%d").date()
        except ValueError:
            raise ValueError("La fecha debe estar en formato YYYY-MM-DD.")

    def __str__(self):
        return f"Fertilizante: {self.nombre}, Precio: ${self.precio:.2f}, Registro ICA: {self.registro_ica}, " \
               f"Frecuencia: {self.frecuencia_aplicacion} días, Última aplicación: {self.ultima_aplicacion}"
